...
def main():
    ...
    parser.add_argument('--epochs', default=6, type=int, required=False, help='训练循环')
    parser.add_argument('--batch_size', default=8, type=int, required=False, help='训练batch size')
    parser.add_argument('--lr', default=1.5e-4, type=float, required=False, help='学习率')
    parser.add_argument('--warmup_steps', default=2000, type=int, required=False, help='warm up步数')
    parser.add_argument('--log_step', default=1, type=int, required=False, help='多少步汇报一次loss')
    # parser.add_argument('--stride', default=768, type=int, required=False, help='训练时取训练数据的窗口步长')
    parser.add_argument('--stride', default=16, type=int, required=False, help='训练时取训练数据的窗口步长')
    ...
    for epoch in range(epochs):
        ...
        for i in x:
            ...
            # save model
            print('saving model for piece {}'.format(piece_num + 1))
            if not os.path.exists(output_dir + 'model_piece'):
                os.mkdir(output_dir + 'model_piece')
            model_to_save = model.module if hasattr(model, 'module') else model
            model_to_save.save_pretrained(output_dir + 'model_piece')
            piece_num += 1
